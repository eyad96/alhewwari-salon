...
import { Scissors, Mail, Lock, User, KeyRound, AlertCircle, ArrowRight, Loader2 } from 'lucide-react'
...
const SignUpPage: React.FC = () => {
  ...
  const handleOAuthSignUp = async (provider: 'oauth_google' | 'oauth_facebook') => {
    if (!isLoaded) return
    const type = provider === 'oauth_google' ? 'google' : 'facebook'
    setIsSocialLoading(type)
    setError('')
    ...
    try {
      await signUp.authenticateWithRedirect({
        ...
        redirectUrlComplete: import.meta.env.VITE_CLERK_AFTER_SIGN_UP_URL || '/dashboard',
      })
    } catch (err: any) {
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  >
                    ...
                    <span>{isSocialLoading === 'google' ? 'جاري التحويل...' : 'جوجل'}</span>
                  </button>
                  ...
                  >
                    ...
                    <span>{isSocialLoading === 'facebook' ? 'جاري التحويل...' : 'فيسبوك'}</span>
                  </button>
                </div>
